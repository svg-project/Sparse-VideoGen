...
def cosmos_token_reorder_to_token_major(tensor, fix_len, reorder_len, reorder_num_frame, frame_size):
...
def cosmos_token_reorder_to_frame_major(tensor, fix_len, reorder_len, reorder_num_frame, frame_size):
...
@triton.jit
def cosmos_sparse_head_placement_kernel(
    query_ptr,
    key_ptr,
...
def cosmos_sparse_head_placement(
    ...
    cosmos_sparse_head_placement_kernel[grid](
        query,
        key,
        value,
        query_out,
...
def ref_cosmos_sparse_head_placement(query, key, value, best_mask_idx, context_length, num_frame, frame_size):
    cfg, num_heads, seq_len, head_dim = query.shape
    assert seq_len == context_length + num_frame * frame_size
    ...
    query_out[best_mask_idx == 1], key_out[best_mask_idx == 1], value_out[best_mask_idx == 1] = (
        cosmos_token_reorder_to_token_major(
...
def test_cosmos_sparse_head_placement():
    ...
    ref_query_out, ref_key_out, ref_value_out = ref_cosmos_sparse_head_placement(
...
def benchmark_cosmos_sparse_head_placement():
    import time
    ...
    start = time.time()
    for _ in range(all_iter):
        ref_cosmos_sparse_head_placement(query, key, value, best_mask_idx, context_length, num_frame, frame_size)
    torch.cuda.synchronize()
    end = time.time()
...
@triton.jit
def cosmos_hidden_states_placement_kernel(
...
def cosmos_hidden_states_placement(hidden_states, hidden_states_out, best_mask_idx, context_length, num_frame, frame_size):
    ...
    cosmos_hidden_states_placement_kernel[grid](
        hidden_states,
        hidden_states_out,
        best_mask_idx,
...
def ref_cosmos_hidden_states_placement(
    ...
    # Temporal
    output_hidden_states[best_mask_idx == 1] = cosmos_token_reorder_to_frame_major(
        hidden_states[best_mask_idx == 1].unsqueeze(0), context_length, num_frame * frame_size, num_frame, frame_size
    ).squeeze(0)


def test_cosmos_hidden_states_placement():
    ...
    cosmos_hidden_states_placement(hidden_states, hidden_states_out1, best_mask_idx, context_length, num_frame, frame_size)
...
def benchmark_cosmos_hidden_states_placement():
    import time
...
if __name__ == "__main__":
    test_cosmos_sparse_head_placement()
    benchmark_cosmos_sparse_head_placement()
    test_cosmos_hidden_states_placement()
    benchmark_cosmos_hidden_states_placement()
