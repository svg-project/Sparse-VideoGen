...
from .constants import NEGATIVE_PROMPT, PRECISION_TO_TYPE, PROMPT_TEMPLATE
...
class Inference(object):
    ...
    def from_pretrained(cls, pretrained_model_path, args, device=None, **kwargs):
        ...
        if args.ulysses_degree > 1 or args.ring_degree > 1:
            ...
            assert dist.get_world_size() == args.ring_degree * args.ulysses_degree, "number of GPUs should be equal to ring_degree * ulysses_degree."
    ...
    def load_state_dict(args, model, pretrained_model_path):
        ...
        if bare_model is False:
            if load_key in state_dict:
                ...
                raise KeyError(f"Missing key: `{load_key}` in the checkpoint: {model_path}. The keys in the checkpoint " f"are: {list(state_dict.keys())}.")
...
class HunyuanVideoSampler(Inference):
    def __init__(self, args, vae, vae_kwargs, text_encoder, model, text_encoder_2=None, pipeline=None, use_cpu_offload=False, device=0, logger=None, parallel_args=None):
        super().__init__(
            args, vae, vae_kwargs, text_encoder, model, text_encoder_2=text_encoder_2, pipeline=pipeline, use_cpu_offload=use_cpu_offload, device=device, logger=logger, parallel_args=parallel_args
        )
    ...
    def get_rotary_pos_embed(self, video_length, height, width):
        ...
        if isinstance(self.model.patch_size, int):
            assert all(s % self.model.patch_size == 0 for s in latents_size), (
                f"Latent size(last {ndim} dimensions) should be divisible by patch size({self.model.patch_size}), " f"but got {latents_size}."
            )
    ...
    def get_prompt_mask(
        ...
        **kwargs,
        ...
        if width <= 0 or height <= 0 or video_length <= 0:
            raise ValueError(f"`height` and `width` and `video_length` must be positive integers, got height={height}, width={width}, video_length={video_length}")
        ...
        **kwargs,
        ...
        if seed is None:
            ...
            if len(seed) == batch_size:
                ...
                raise ValueError(f"Length of seed must be equal to number of prompt(batch_size) or " f"batch_size * num_videos_per_prompt ({batch_size} * {num_videos_per_prompt}), got {seed}.")
        else:
        ...
        # Scheduler
        # ========================================================================
        scheduler = FlowMatchDiscreteScheduler(shift=flow_shift, reverse=self.args.flow_reverse, solver=self.args.flow_solver)
