...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, t):
        ...
        x = flash_attention(q=q, k=k, v=v, k_lens=seq_lens, window_size=self.window_size, layer_idx=self.layer_idx, timestep=t)
...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self, cross_attn_type, dim, ffn_dim, num_heads, window_size=(-1, -1), qk_norm=True, cross_attn_norm=False, eps=1e-6, layer_idx=0):
        super().__init__()
        self.dim = dim
    ...
    def forward(
        ...
        # self-attention
        y = self.self_attn(y, seq_lens, grid_sizes, freqs, t)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([WanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads, window_size, qk_norm, cross_attn_norm, eps, layer_idx=layer_idx) for layer_idx in range(num_layers)])
    ...
    def forward(
        ...
        # arguments
        kwargs = dict(e=e0, t=t, seq_lens=seq_lens, grid_sizes=grid_sizes, freqs=self.freqs, context=context, context_lens=context_lens)
